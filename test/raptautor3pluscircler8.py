...
# 全局变量用于控制防休眠线程
stop_keep_awake_event = None
keep_awake_thread = None

def start_keep_awake_thread(activity_interval=55):
    """
    启动独立的防休眠线程
    """
    global stop_keep_awake_event, keep_awake_thread
    
    stop_keep_awake_event = threading.Event()
    
    def keep_awake_worker():
        """后台线程函数，定期模拟用户活动防止休眠[1,4](@ref)"""
        try:
            logging.info(f"启动防休眠后台线程，活动间隔: {activity_interval}秒")
            while not stop_keep_awake_event.is_set():
                try:
                    # 获取当前鼠标位置
                    ...
                    logging.debug(f"执行防休眠鼠标移动 at {time.strftime('%Y-%m-%d %H:%M:%S')}")
                except Exception as e:
                    logging.warning(f"防休眠活动执行失败: {str(e)}")
                
                # 等待指定间隔或被停止
                stop_keep_awake_event.wait(activity_interval)
                
        except Exception as e:
            logging.error(f"防休眠线程错误: {str(e)}")
    
    keep_awake_thread = threading.Thread(
        target=keep_awake_worker, 
        daemon=True
    )
    keep_awake_thread.start()

def stop_keep_awake_thread():
    """
    停止防休眠线程
    """
    global stop_keep_awake_event
    if stop_keep_awake_event:
        stop_keep_awake_event.set()
        logging.info("防休眠线程已停止")
...
def wait_and_click(image_path, timeout=10, interval=1, max_retries=3, click=True):
    ...
    activity_interval = 55  # 防休眠活动间隔（秒），略小于典型的1分钟休眠阈值[1](@ref)
    ...
    while retry_count < max_retries and (time.time() - start_time) < timeout:
        ...
        if remaining_time > 0 and retry_count < max_retries - 1:
            logging.debug(f"未找到图像: {image_path}，{min(interval, remaining_time):.1f}秒后重试...")
            time.sleep(min(interval, remaining_time))
        retry_count += 1
    
    logging.info(f"未找到图像: {image_path} (达到最大重试次数或超时)")
    return False
...
def autoprint(file_path):
    ...
    try:
        ...
        try:
            if print_started:
                # 确保焦点在RAPT窗口上
                RAPT_img = get_resource_path('RAPT.PNG')
                RAPT_location = pyautogui.locateOnScreen(RAPT_img, grayscale=True, confidence=0.75)
                pyautogui.leftClick(pyautogui.center(RAPT_location))
                time.sleep(2)
                #关闭当前RATP文件
